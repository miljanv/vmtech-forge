...
The result must feel like a Dribbble editorial brand site: warm paper canvas, oversized serif, photography in a large rounded frame, generous whitespace, and a palette taken from the craft itself — never a SaaS template, never indigo-on-white, never three equal generic cards, never white text on a cream panel.
Choose colors with accessible contrast (dark ink on paper, cream type only on inverted dark bands). Pick fonts only from the provided supported pairing ids.
Prefer editorial-split heroes, numbered hairline sections, magazine product catalogs, and image treatments that feel tactile (film grain, warm grade).
...
Build a complete homepage that feels like a $10k Dribbble editorial site: split hero with dark type on paper and a large rounded photo, numbered trust rows, magazine product catalog, story, process timeline, overlapping gallery, and an inverted contact band.
...
Prefer hero variant "editorial-split" and gallery variant "offset-editorial" or "filmstrip". Set animation to fade-up or reveal on every visible section. Never put light copy over a light background.
`.trim();
